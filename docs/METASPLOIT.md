...
All fields live under the `[metasploit]` section of `config/default.toml`.
...
**Output:** Compact table with module path, rank, and short description.
...
The handler polls for completion with exponential backoff (2s to 16s, up to 10 attempts) and reports any new sessions created.
...
**Output:** Module UUID, completion status, and result data (truncated to 3000 chars). Polls with exponential backoff.
...
| `session_id` | number | for interact/stop/compatible_modules | Session ID |
| `command` | string | for interact | Command to run in the session |
...
**Output:** Module UUID, completion status, and result data (truncated to 2000 chars). Polls with exponential backoff up to 8 attempts.
...
All tools that accept a `target` parameter pass it through `safety::validate_target()` before any RPC call. The same allowlist validation used by nmap, nuclei, and other tools applies here.

### Layer 3: Module Blocklist

The `blocked_modules` config field accepts substring patterns. Before executing any module (exploit, auxiliary, or post), the client checks the module path against every pattern. If any match is found, execution is denied:
...
3. **Changed parameters:** If the second call has different parameters, it replaces the pending hash — execution does not proceed.
...
The handler polls for up to ~5 minutes (10 attempts with exponential backoff from 2s to 16s). If the exploit takes longer, check `msf_sessions` for any sessions that were created, or use `msf_auxiliary` to verify the target is reachable first.
